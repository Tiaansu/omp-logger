...
#define CHECK_ARGS(n)                                                                                                                                  \
	...
	{                                                                                                                                                  \
		...
		OmpLoggerComponent::Get()->getCore()->logLn(LogLevel::Error, "String formatted incorrectly - parameter: %d, total: %d, format: %s", arg, args, formatStr); \
		return 0;                                                                                                                                      \
	}
...
reswitch:
		...
		case 'a':
		{
			...
			if (!cptr)
			{
				OmpLoggerComponent::Get()->getCore()->logLn(LogLevel::Error, "Invalid vector string handle provided");
				return 0;
			}
			cell* ptr = reinterpret_cast<cell*>(*cptr);
			if (!ptr)
			{
				OmpLoggerComponent::Get()->getCore()->logLn(LogLevel::Error, "Invalid vector string handle provided (%d)", *cptr);
				return 0;
			}
...
AmxStringFormatter::AmxStringFormatter(cell const* format, AMX* amx, cell const* params, int paramOffset) noexcept
{
	...
	else
	{
		...
		if (paramOffset <= count)
		{
			...
			OmpLoggerComponent::Get()->getCore()->logLn(LogLevel::Warning, "Insufficient specifiers given: \"%s\" does not format %u parameters.", fmt, diff);
		}
	}
}